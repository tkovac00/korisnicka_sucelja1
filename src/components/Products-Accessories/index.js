...
class FilteredAccessories extends Component {
    ...
    constructor(props) {
        ...
        this.fuse = new Fuse(props.accessories, options);
    }
    ...
    render() {
        ...
        let searchResults = Lodash.orderBy(accessories);

        if (search.length > 0) {
            searchResults = this.fuse.search(search);
        }
        const accessoriesList = searchResults.map((data) => {
        ...
        return (
            ...
            <h1 style={{ textAlign: 'center', marginTop: '-20px', opacity: '0.8' }}>Accessories</h1>
            ...
            <div className={styles.Main}>{accessoriesList}</div>
