...
class FilteredClothes extends Component {
    ...
    constructor(props) {
        ...
        this.fuse = new Fuse(props.clothes, options);
    }
    ...
    render() {
        ...
        let searchResults = Lodash.orderBy(clothes);

        if (search.length > 0) {
            searchResults = this.fuse.search(search);
        }
        const clothesList = searchResults.map((data) => {
        ...
        return (
            ...
            <div className={styles.Main}>{clothesList}</div>
